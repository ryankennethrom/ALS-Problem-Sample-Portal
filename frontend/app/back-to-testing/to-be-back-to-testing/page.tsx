...
function rowValue(item: TestingSample, directValue: unknown, fieldKeys: string[]) {
...
export default function ToBeBackToTestingPage() {
  const { requestChangeReason, changeReasonModal } = useChangeReasonModal();
  const [items, setItems] = useState<TestingSample[]>([]);
  ...
  async function load() {
    const data = await api('/problem-samples/to-be-back-to-testing/');
  ...
  useEffect(() => {
    load()
      .catch(error => setError(error instanceof Error ? error.message : 'Failed to load samples to be returned to testing.'))
  ...
  async function markBackToTesting() {
    ...
    if (!confirm(`Mark ${selectedCount} selected ${noun} as Back to testing?`)) return;

    setUpdating(true);
    setError('');
    try {
      await api('/problem-samples/bulk-back-to-testing/', {
        method: 'POST',
        headers: changeReasonHeaders(reason),
        body: JSON.stringify({ problem_ids: Array.from(selected) }),
        successMessage: `${selectedCount} ${noun} marked Back to testing.`,
        errorMessage: 'Could not update the selected problem samples',
      });
      await load();
    } catch (error) {
      setError(error instanceof Error ? error.message : 'Could not update the selected problem samples.');
    } finally {
      setUpdating(false);
    }
  }
  ...
  return <div>
    <div className="page-toolbar">
      <div>
        <div className="eyebrow">Back To Testing</div>
        <h1 className="page-heading" style={{marginBottom: 2}}>To be back to testing</h1>
        <div className="muted table-description">Select one or more problem samples that are ready to return to testing, then mark them back to testing together.</div>
      </div>
      <div className="toolbar-actions">
        <button className="button" type="button" onClick={markBackToTesting} disabled={!selectedCount || updating}>
          {updating ? 'Updating…' : selectedCount ? `Back to Testing (${selectedCount})` : 'Back to Testing'}
        </button>
      </div>
    </div>
    ...
    <section className="panel panel-blue shipping-panel">
      <div className="shipping-toolbar">
        <div className="search-grid table-search-grid compact-table-search-grid advanced-search-grid" style={{flex: 1}}>
          <div className="field search-wide">
            <label htmlFor="testing-search">Search samples</label>
            <input id="testing-search" className="input" value={query} onChange={event => setQuery(event.target.value)} placeholder="Problem #6, container, client, tracking number…" />
          </div>
        ...
        <div className="shipping-summary">
          <strong>{query.trim() || advancedFilters.length ? filtered.length : items.length}</strong> sample{(query.trim() || advancedFilters.length ? filtered.length : items.length) === 1 ? '' : 's'} to be back to testing
      ...
      <div className="data-table-wrap shipping-table-wrap">
        <table className="data-table shipping-table">
          <thead>
            <tr>
              <th className="shipping-select-column">
                <input type="checkbox" aria-label="Select all visible samples" checked={allVisibleSelected} onChange={toggleAllVisible} disabled={filtered.length === 0 || updating} />
              </th>
              <th>Problem ID</th>
              <th>Date Created</th>
              <th>Table</th>
          ...
          <tbody>
            {!loading && filtered.length === 0 && <tr><td colSpan={10} className="empty-table">{items.length ? 'No samples match your search.' : 'No problem samples are waiting to go back to testing.'}</td></tr>}
            ...
            {!loading && filtered.map(item => <tr key={item.id} className={selected.has(item.id) ? 'shipping-row-selected' : ''}>
              <td className="shipping-select-column"><input type="checkbox" aria-label={`Select problem ${item.problem_number}`} checked={selected.has(item.id)} onChange={() => toggleOne(item.id)} disabled={updating} /></td>
              ...
              <td><span className="badge">To be back to testing</span></td>
            </tr>)}
          </tbody>
        </table>
      </div>
