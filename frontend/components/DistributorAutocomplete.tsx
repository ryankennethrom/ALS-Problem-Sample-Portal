...
export default function DistributorAutocomplete({
  ...
  label,
  required = false,
  id = 'distributor-autocomplete',
  placeholder = 'Start typing a distributor company…',
}: {
  value: unknown;
  ...
  const [suggestions, setSuggestions] = useState<DistributorSuggestion[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setLoading(true);
      try {
        const data = await api(`/customers/distributors/suggest/?q=${encodeURIComponent(query)}`);
  ...
  function choose(item: DistributorSuggestion) {
  ...
  return <div className="field distributor-field">
    ...
    <div className="distributor-autocomplete">
      ...
      {open && <div className="distributor-suggestions" role="listbox">
        ...
        </button>) : <div className="distributor-empty">No distributor companies found</div>}
      </div>}
    </div>
    <div className="muted result-meta">Fuzzy suggestions are limited to customer records where CoyType is Distributor.</div>
  </div>;
}
